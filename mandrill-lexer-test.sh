...
_SC git pull
(_SC git checkout lexer) || { echo "[WARNING] Cannot find specific branch, use main branch for test."; }
_SC cd ..
...
for name in ${names[@]}; do
    ...
    _SC mkdir $name
    _SC cd $name
    (_SC git clone --branch lexer --depth 1 "git@bitbucket.org:${name}/mandrill2025.git") ||\
    (_SC git clone --branch lexer --depth 1 "git@bitbucket.org:${name}/mandrill2025.git") ||\
    (_SC git clone --branch lexer --depth 1 "git@bitbucket.org:${name}/mandrill2025.git") ||\
    { echo "[ERROR] cannot clone code, goto next student..."; echo $name "0/-8" >>$statistics; continue; }
